...
measurements per plot along 5m long Unispec transect as marked on maps.
Scans are made 0.5m into plot (given monopod arm length). This is the
default order in which the "unispec_key.csv" file is organized, telling
you which measurement belongs to which plot.
...
2.  **Minimize shadows by measuring close to Solar Noon (13:00).** Try
    ...
    a.  Day 1: up the hill to MAT, LMAT, WSG-inlet in the morning; drive
        to p-loop WSG-outlet, DHT afternoon
    b.  Day 2: boat across the lake to NANT, MNAT in the morning; drive
        to SHB1, SHB2, HST in the afternoon
...
3.  **Monopod** - 2m metal cross. If you're going to the SHRUB sites
    bring the **wooden Extension**, Attachments (3 screws, 3 nuts), and
    2 x 7/16" **wrenches**, and grey **right-angle level.**
...
2.  Access the the **set up menu** by pressing "1"
    ...
    -   In the *"File Prefix"* box, enter the site name abbreviation
        (Ex: `mat06_` )
...
5.  Pack the unispec in the bottom of a field back pack

    -   You can find backpacks on the rack in the entryway to the lab
        with the drying ovens. Typically, we use the red backpack
    -   **If you are going across the lake make sure you put the unispec
        and other equipment into a dry bag!**

6.  Pack the silver clipboard containing...

    -   two cables (upward and downward facing sensors)
    -   white reference
    -   pencil
    -   map(s) of site(s)
...
8.  Don't forget your own supplies!
...
1.  **Wait 10 min for machine to warm up.** Good things to do while
    waiting:
...
2.  **Set scan preferences (press 1).**
    ...
    c.  Set file prefix: **SITE\_** (e.g. MAT\_ or DHT\_ don't forget
        the underscore!)
    ...
    g.  **Set Default Data Directory and File** to Folder named by
        **DATE** created in (b). Save as **"Default.spu"** within folder

3.  **Perform throwaway scan (press 2).** Initializes channels after
    instrument has been turned off. 

4.  **Perform a dark scan (press 3).** Fold over case to block all light
    from detectors.
...
5.  **Attach foreoptics to UniSpec.** 

    a.  Make sure A is the upward facing and has the Cosine Receptor
        (small black rectangle) 
    b.  Make sure B is the downward facing and has the long metal tube
        to adjust FOV

6.  **Perform reference scan (press 2)**

    a.  Examine disc to make sure it is clean, sandpaper surface if
        necessary. DO NOT TOUCH the circular surfaces to avoid
        contamination with oils.

    b.  Place in holder beneath down optic -- make sure it is close but
        no shadow on disc and it is at the same angle to sun as the
        Cosine Receptor.

    c.  Take 3 measurements at right angles facing direction similar to
        real measurements.

        i.  Channel A max should be around 50,000 A/D.
        ii. Channel A and Channel B should be fairly similar.

7.  **Prep for real measurements:** 

    a.  Change scans to Average back to 1. 
    b.  Write down the file name for Throwaway (00), Dark (01), and
        Reference (02-04) scans. 

8.  **Make real measurements (press 2)**
    ...
    e.  Note which blocks and plots go to each set of 5 files in
        notebook.

9.  **Transfer data files and update key THE SAME DAY YOU TAKE
    MEASUREMENTS.** See data procedure below
...
1.  **Remove PCIMA card from Unispec DC \>\> insert into reader with USB
    connection to computer.** Be careful to align the PCIMA card
    properly to avoid bending any of the metal prongs on the connector.
    If this occurs, you can straighten with pliers & reinsert card to
    realign. 

2.  **Transfer data folders to computer.** Move folders named with
    [YEAR-MONTH-DAY} containing SITE_XXXXX.spu files to appropriate
    location in OneDrive/Dropbox.

3.  **Fill out Unispec Key,** add notes & file numbers to
    **[YEAR_unispec_key.csv]** excel sheet.

4.  **Open ["process_unispec_files.Rmd"] R Markdown notebook in
...
6.  **Add data to Shiny App** to compare with other years by moving
...
[BACK to home
page](https://lter-arc.github.io/DATA_COLLECTION_and_MANAGEMENT/)
